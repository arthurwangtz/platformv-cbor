...
class Encoder extends stream.Transform {
  ...
  constructor(options) {
    const opts = Object.assign({}, options, {
      ...
      highWaterMark: 0x20000 // Increase watermark from default(16KB) to 128KB
    })
    super(opts)
  ...
  _pushInt(obj, mt, orig) {
    ...
    switch (false) {
      ...
      case !(obj <= 0xff):
        r = this._pushUInt8(m | NUMBYTES.ONE)
        r = this._pushUInt8(obj) && r
        return r;
      case !(obj <= 0xffff):
        r = this._pushUInt8(m | NUMBYTES.TWO)
        r = this._pushUInt16BE(obj) && r
        return r;
      case !(obj <= 0xffffffff):
        r = this._pushUInt8(m | NUMBYTES.FOUR)
        r = this._pushUInt32BE(obj) && r
        return r;
      case !(obj <= Number.MAX_SAFE_INTEGER):
        r = this._pushUInt8(m | NUMBYTES.EIGHT)
        r = this._pushUInt32BE(Math.floor(obj / SHIFT32)) && r;
        r = this._pushUInt32BE(obj % SHIFT32) && r
        return r;
      default:
        if (mt === MT.NEG_INT) {
